...
test_preds <- test_preds %>% 
  mutate(run_label = factor(.data$run_id, labels = RUN_LABELS, levels = names(RUN_LABELS)),
         run_group = if_else(.data$run_id == "phylogeny", "", "ACE2 sequence representations"),
...
random_accuracy <- sum(infection_data$infected == "True") / nrow(infection_data)
...
p_overall <- ggplot(overall_accuracies, aes(x = run_label, y = accuracy)) +
  ...
  geom_hline(yintercept = random_accuracy, linetype = 2, colour = "grey10") +
  scale_y_continuous(limits = c(0, 1), expand = expansion(0.02)) +
  labs(x = "Predictors used", y = "Accuracy") +
  coord_flip() +
...
# ---- Output --------------------------------------------------------------------------------------
p_combined <- plot_grid(p_overall, p_sens, p_spec, 
...
focal_models <- RUN_LABELS[c("all_features", "phylogeny")]
